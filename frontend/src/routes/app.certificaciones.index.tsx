    ...
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card className="p-4">
          <form onSubmit={handleAdd} className="flex flex-wrap gap-3 items-end">
            <div>
              ...
              <Input id="cname" name="cname" placeholder="AWS Solutions Architect" required />
            </div>
            <div>
              <Label htmlFor="cissuer">Emisor *</Label>
              <Input id="cissuer" name="cissuer" placeholder="Amazon" required />
            </div>
            <div>
              <Label htmlFor="curl">URL credencial</Label>
              <Input id="curl" name="curl" placeholder="https://..." />
            </div>
      ...
      {certs.length === 0 ? (
        <Card className="p-10 text-center text-muted-foreground">
          <p>No tienes certificaciones todavía.</p>
        </Card>
      ) : (
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {certs.map((c) => (
            <Card key={c.id} className="p-4">
              ...
              {c.issue_date && (
                <p className="text-xs text-muted-foreground mt-1">Emitida: {c.issue_date}</p>
              )}
              {c.credential_url && (
                <a
