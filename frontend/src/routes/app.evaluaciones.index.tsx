...
function Evals() {
  ...
  if (loading)
    return <div className="p-10 text-center text-muted-foreground">Cargando evaluaciones...</div>;

  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <PageHeader
        title="Evaluaciones técnicas"
        description="Valida tus skills y desbloquea certificaciones."
      />
      <Tabs defaultValue="catalogo">
        <TabsList>
          <TabsTrigger value="catalogo">Catálogo ({evals.length})</TabsTrigger>
          <TabsTrigger value="historial">Historial ({results.length})</TabsTrigger>
        </TabsList>
                ...
                .map((ev) => (
                  <Card key={ev.id} className="p-5">
                    ...
                    <Button asChild size="sm" variant="outline" className="w-full mt-3">
                      ...
                      >
                        <ChevronRight className="h-4 w-4 mr-1" /> Tomar
                      </a>
                    </Button>
                  </Card>
                ))}
        ...
        <TabsContent value="historial" className="mt-5">
          {results.length === 0 ? (
            <Card className="p-10 text-center text-muted-foreground">
              <p>No has completado ninguna evaluación.</p>
            </Card>
          ) : (
            <div className="space-y-3">
              {results.map((r) => (
                <Card key={r.result.id} className="p-4 flex items-center justify-between">
                  ...
                  >
                    {r.result.passed ? `Aprobado (${r.result.score})` : `${r.result.score}`}
                  </Badge>
                </Card>
              ))}
            </div>
